  ...
  <div class="flex min-h-screen">
    <Transition name="splash-fade">
      ...
      >
        ...
        <div class="relative z-10 text-center px-6" data-tour="welcome-text">
          <p class="text-[11px] uppercase tracking-[0.24em] text-emerald-300/80 mb-3">Tu actividad musical, clara y al instante</p>
          <h1 class="welcome-brand text-4xl sm:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent">
            Skippify
          </h1>
    ...
    <div class="flex-1 min-w-0">
      ...
      <AppTour
        ...
        @toggle-sidebar="handleTourSidebar"
      />
    </div>
  </div>
</template>
...
function handleTourStepChange (index) {
  showSplash.value = showTour.value && index === 0
}

function handleTourSidebar (open) {
  sidebarOpen.value = !!open
}

function handleOpenRouteFromNotification (path) {
...
onMounted(async () => {
  await notif.checkAndInit(setNowPlaying, handleOpenRouteFromNotification)
...
.welcome-brand {
  background-image: linear-gradient(115deg, #ffffff 0%, #d1fae5 28%, #67e8f9 52%, #86efac 78%, #ffffff 100%);
  background-size: 220% 220%;
  animation: welcome-gradient-shift 4.8s ease-in-out infinite;
}
