<template>
  ...
  >
    ...
    <nav class="flex-1 py-3 flex flex-col gap-1 px-2">
      ...
      <router-link to="/claude" custom v-slot="{ isActive, navigate }">
        <button
          data-tour="claude-nav"
          :title="collapsed ? 'Claude AI' : ''"
          class="group relative flex items-center gap-3 rounded-xl transition-all duration-150 w-full"
          :class="[
            collapsed ? 'justify-center px-0 py-3.5' : 'px-3.5 py-3.5',
            isActive
        ...
        >
          <span
            class="flex-shrink-0 flex h-8 w-8 items-center justify-center rounded-lg transition-colors"
            :class="isActive ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-500 group-hover:text-slate-300'"
          >
            <span class="text-lg">🤖</span>
          </span>
          <Transition name="label">
            <div v-if="!collapsed" class="text-left overflow-hidden whitespace-nowrap">
              <p class="text-base font-semibold leading-none">Claude AI</p>
              <p class="text-[10px] mt-0.5 text-slate-500">Chat con Anthropic</p>
            </div>
          </Transition>
          <span
            v-if="collapsed"
