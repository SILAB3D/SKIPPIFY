...
export function useNotifListener () {
  ...
  const { state: features, initializeNativeFeatures, applyNativeFeatureConfig } = useFeatures()
   ...
   */
  async function checkAndInit (onNowPlaying, onOpenRoute) {
    ...
    try {
      await NL.addListener('featureConfigChanged', (data) => {
        applyNativeFeatureConfig(data)
      })
    } catch { /* ignored */ }

    try {
      await NL.addListener('openRoute', (data) => {
        const route = (data?.route || '').toString().trim()
        if (route) {
          onOpenRoute?.(route)
        }
      })
    ...
    try {
      if (NL.consumePendingOpenRoute) {
        const pending = await NL.consumePendingOpenRoute()
        const route = (pending?.route || '').toString().trim()
        if (route) {
          onOpenRoute?.(route)
        }
      }
  ...
  async function setupTrackListener (NL) {
    await NL.addListener('spotifyTrack', async (data) => {
      ...
      const isContinuationByProgress =
        Number.isFinite(progressMs) &&
        durationMs > 0 &&
        progressMs >= Math.ceil(durationMs * MIN_REGISTER_PROGRESS_RATIO)
      ...
      if (features.skipDuplicates && playing && track && artist && trackChanged && nearStart && !isContinuationByProgress) {
